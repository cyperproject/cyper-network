...
export default function AllProblem({ role }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await api.get(`/Problem/GetAllProblemsToUser`, {
          headers: {
  ...
  const columns = [
    ...
    { field: 'Description_Problems', headerName: 'Problem description', width: 450 },
  ];

  return (
    <div style={{ maxHeight: 600, width: '60%', textAlign: "center", border: "2px solid #4e73df" }}>
