...
import Style from "./DeleteRole.module.css";
...
export default function DeleteRole() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await api.delete(`/Admins/RemoveUserFromRole/${formData.UserNameOrID}/${formData.RoleName}`,{
        headers: {
    ...
    <>
      <div className={Style.contactForm}>
        <form onSubmit={handleSubmit} className={Style.form}>
          <h2 className={Style.h2}>Delete Role</h2>
